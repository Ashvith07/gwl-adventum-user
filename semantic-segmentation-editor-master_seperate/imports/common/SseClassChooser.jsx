...
export default class SseClassChooser extends SseToolbar {
    ...
    constructor(props) {
        // localStorage.setItem("counters","0");
    ...
    filter(e) {
        this.setState({ filterValue: event.target.value ? event.target.value : ' ' })
    }
    ...
    render() {
        ...
        // console.log(filterValue, 'this is filter')
        return (


            <div className="sse-class-chooser vflex scroller"
                style={{ "backgroundColor": "#393536", "padding": "5px 5px 0 0", width: "15%", }}>
                ...
                {this.state.soc.descriptors.map((objDesc, idx) => {
                    ...
                    // console.log(JSON.stringify(objDesc));

                    const isSelected = objDesc.classIndex == this.state.activeClassIndex;
                    return (
                        <div>
                            {
                                (this.state.filterValue && objDesc.label.toLowerCase().indexOf(filterValue) >= 0) ?
