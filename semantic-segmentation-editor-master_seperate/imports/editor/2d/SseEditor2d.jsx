...
export default class SseEditor2d extends React.Component {
    ...
    deletePolygon(path) {
        ...
        // console.log(this.currentSample.objects)
        path.remove();
        this.saveData();
    ...
    drawAnnotations() {
        ...
        // this.anomaly_arr.pop();
        // this.anomaly_redo.pop();
    ...
    setColor(path, color) {
        // alert('twice')
        console.log(path)
        // alert(this.state.anomaly_name);
        if(this.state.color_push != color)
        {
            this.setState({
                color_push:color,
                index:path.hasOwnProperty('feature')?path.feature.classIndex:this.state.index
            })
            // this.anomaly_arr=[];
             console.log("this.state",this.state.index)
             if(this.state.index=='')
             {
                 this.state.index=0;
            ...
            this.anomaly_arr.push({ 'anomaly_hexcode': this.state.color_push,'index': this.state.index })
            this.anomaly_redo.push({ 'anomaly_hexcode': this.state.color_push,'index': this.state.index })
            
            console.log("anomaly_array",this.anomaly_arr);
            console.log("redo_array",this.anomaly_arr)
           
        }
        
        // localStorage.setItem("Annonated",this.anomaly_arr)
        // console.log(this.state.anomaly_name)
        // console.log("after pushing color",this.anomaly_arr);
...
*/
    setupMessages() {
        ...
        // alert("messages");
            ...
            this.setState({
                anomaly_name: descriptor.label,
                index: descriptor.classIndex
            })

            console.log("desc",descriptor.label);
            console.log("desc",descriptor.classIndex);

            // this.anomaly_arr.push({"anomaly_name":descriptor.label});
            // console.log("Array when anomaly pushed",this.anomaly_arr);
        ...
        this.onMsg("reset-end", () => {
            ...
            console.log("anomaly_arr===", this.anomaly_arr);
            location.reload();
        });
...
*/
    ...
    delete() {
        ...
        if (selectedPoints.length > 0) {
            ...
            console.log("selected polygon", selectedPolygons[0].feature.classIndex);
            this.setState({
                index: selectedPolygons[0].feature.classIndex
            });
            // let array=this.anomaly_arr;
            for (let i = 0; i < this.anomaly_arr.length; i++) {

                console.log("myarray===", this.anomaly_arr[i].index)
                if (this.anomaly_arr[i].index === this.state.index) {
                    this.anomaly_arr.pop(this.anomaly_arr[i].index);
                    console.log("after deletion", this.anomaly_arr)
                }
    ...
    undo() {
        // alert("clicked")
        this.cancel();
        let ustate = this.undoRedo.undo();
        console.log("length==", ustate)
        console.log("Ustate===", ustate)
        // console.log("array in undo", this.anomaly_arr);
        // for (let i = 0; i < this.anomaly_arr.length; i++) {
        //     console.log(this.anomaly_arr[i].index)
            

        //     if (this.anomaly_arr[i].index == ustate.objects[0].classIndex) {
        //          console.log("hie")
        //         this.anomaly_arr.splice(i,1);

        //     }


        // }
        // // this.anomaly_arr = [...new Set(this.anomaly_arr)];
        // console.log("array after pop==",this.anomaly_arr);

        if (ustate) {
       
        this.anomaly_arr.pop();
        console.log("array after ",this.anomaly_arr);

            // alert("inside u state")
    ...
    redo() {
        ...
        console.log("inside redo",ustate.objects);


        if (ustate) {
          this.anomaly_arr.push(this.anomaly_redo[this.anomaly_redo.length-1]);
          this.anomaly_redo.pop();

          console.log("anomaly arr==",this.anomaly_arr);
          console.log("anomaly redo==",this.anomaly_redo);
            this.setCurrentSample(ustate);
            this.drawAnnotations();
            this.saveData(true);
        } else {
    ...
    download() {
        ...
        let path = location.replace(/%2F/g, '/')
        let path1 = path.replace('/20%', '/')
        let path2 = path1.replace('/%20/g', '')
        let orginal_image_path1 = path2.replace('//', '/');
        let orginal_image_path = orginal_image_path1.substr(0, path.lastIndexOf("/") - 1);
        console.log(orginal_image_path);
        ...
        setTimeout(() => {
            canvas.toBlob((blob) => {
                ...
                // this.fileName.replace(/\.png$/, "_segmentation.png");
                console.log(blob);
                ...
                if(this.anomaly_arr.length>0)
                {
                const data = new FormData();
                data.append('original_filepath', orginal_image_path);

                console.log("after click download deletion", this.anomaly_arr)
                let anomaly_list = JSON.stringify(this.anomaly_arr)
                data.append('Annotated', anomaly_list)
                ...
                alert("Please annonate properly or else reset");
